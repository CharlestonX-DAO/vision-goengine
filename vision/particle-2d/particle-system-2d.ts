 ...
 *  - blending function
 *  - texture
 * Cocos also supports particles generated by Particle Designer (http://particledesigner.71squared.com/).
 * 'Radius Mode' in Particle Designer uses a fixed emit rate of 30 hz. Since that can't be guaranteed in cocos,
 * cocos uses a another approach, but the results are almost identical.
 * cocos supports all the variables used by Particle Designer plus a bit more:
